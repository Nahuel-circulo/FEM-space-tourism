                ...
                <div className="mx-auto grid place-items-center lg:grid-cols-2 lg:grid-rows-3  w-full max-w-md md:max-w-2xl lg:max-w-6xl pt-12">
                            ...
                            <div className="flex  uppercase justify-center whitespace-nowrap gap-4">
                                {
                                    ...
                                    destinations.map((item: IDestination) => (
                                        <button onClick={() => setCurrentDestination(item)} key={item.name} className={`${currentDestination?.name === item.name ? " border-white text-white" : "border-transparent text-gray-300"}  inline-flex items-center uppercase h-10   text-sm     border-b-2  sm:text-base  whitespace-nowrap focus:outline-none`}>
                                            {item.name}
                                        </button>
                                    ))
                                }
                    ...
                    <div className=" w-full p-8 uppercase font-bellefair  grid gap-8 place-items-center border-t-2 border-solid border-gray-500">
                        ...
                        <div className="w-full text-center">
                            ...
                            <h3 className="text-white text-3xl">{currentDestination?.travel}</h3>
                        </div>
                    </div>
                </div>
            </div>
